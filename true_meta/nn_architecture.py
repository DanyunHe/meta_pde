...
#         kernel_size = 5
#         self.layer_dict = nn.ModuleDict()
#         self.layer_dict["conv1d_0", param = nn.Parameter(nn.init.xavier_uniform_(torch.empty(32,1, kernel_size)).double().cuda(),requires_grad = True)),
#         self.register_parameter(name="conv1d_1", param = nn.Parameter(nn.init.xavier_uniform_(torch.empty(32,32, kernel_size)).double().cuda(),requires_grad = True)),
#         self.register_parameter(name="conv1d_2", param = nn.Parameter(nn.init.xavier_uniform_(torch.empty(32,32, kernel_size)).double().cuda(),requires_grad = True)),
#         self.register_parameter(name="bias1d_0", param = nn.Parameter(torch.zeros(num_filters).double().cuda(),requires_grad = True)),
#         self.register_parameter(name="bias1d_1", param = nn.Parameter(torch.zeros(num_filters).double().cuda(),requires_grad = True)),

#         self.register_parameter(name="bias1d_2", param = nn.Parameter(torch.zeros(num_filters).double().cuda(),requires_grad = True)),

#         self.register_parameter(name="linear", param = nn.Parameter(nn.init.xavier_uniform_(torch.empty(480,640)).double().cuda(),requires_grad = True)),
#         self.register_parameter(name='bias', param = nn.Parameter(torch.zeros(480).double().cuda(),requires_grad = True))
...
#         else:
#             for i in range(self.num_conv_layes):
#                 x = F.conv1d(x, weight = self.state_dict()['conv1d_'+str(i)], bias = self.state_dict()['bias1d_'+str(i)])
#                 x = F.relu(x)
#             x = torch.flatten(x,1)
#             # import pdb
#             # pdb.set_trace()
#             out = F.linear(x, weight = self.state_dict()['linear'], bias = self.state_dict()['bias'])
